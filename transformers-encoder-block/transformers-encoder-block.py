...
def layer_norm(x: np.ndarray, gamma: np.ndarray, beta: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    ...
    output1 = gamma * ((x - mean) / np.sqrt(var + eps)) + beta
    return output1
...
def encoder_block(x: np.ndarray, W_q: np.ndarray, W_k: np.ndarray, W_v: np.ndarray,
    ...
    norm=layer_norm(x+attention,gamma1,beta1)
    ffn_out=feed_forward(norm,W1,b1,W2,b2)
    output4=layer_norm(norm+ffn_out,gamma2,beta2)
    return output4